...
export default function ProjectDetails() {
  ...
  const fetchProjectDetails = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('projects').select('*, creators(user_id)').eq('id', projectId).single();

    if (error) {
      console.error(error);
      setError("Could not load the project. It may have been deleted or doesn't exist.");
    } else if (data) {
      setProject(data);
      setIsOwner(data.creators?.user_id === user?.id);
      setEditFormData({
  ...
  const updateProject = async () => {
    ...
    try {
      ...
      setModalError('Failed to update project. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="pb-24 w-full">
      {/* Edit Modal */}
      <Modal isOpen={isEditMode} onClose={() => setIsEditMode(false)} title="Edit Project">
        <form onSubmit={(e) => { e.preventDefault(); updateProject(); }} className="p-7 space-y-6">
          {modalError && <div className="alert-error !p-3">{modalError}</div>}
          <div>
            <label className="field-label">Project Name <span className="text-rose-400">*</span></label>
            <input required type="text" name="project_name" value={editFormData.project_name} onChange={handleInputChange} className="field" placeholder="e.g. Project Alpha" />
          </div>

          <div>
            <label className="field-label">Description</label>
            <textarea rows="3" name="description" value={editFormData.description} onChange={handleInputChange} className="field resize-none" placeholder="What is this project about?"></textarea>
          </div>
          ...
          <div className="pt-6 border-t border-white/10 flex justify-end gap-3 pb-1">
            ...
            <button type="submit" disabled={isSaving} className="btn-primary btn-md">
        ...
        <div className="relative overflow-hidden h-64 lg:h-80 border-b border-white/[0.07]">
          {project.image ? (
            <div className="absolute inset-0 z-0">
              <SafeImage src={project.image} alt={project.project_name} className="w-full h-full object-cover opacity-30" fallback={null} />
        ...
        <div className="max-w-6xl mx-auto px-5 sm:px-8 relative -mt-24 z-10 pb-8">
          <div className="flex flex-col md:flex-row gap-7 md:items-end text-left">
            <div className="gradient-ring rounded-3xl shrink-0 h-36 w-36 bg-ink-900 p-1.5 shadow-card animate-fade-up">
              <div className="w-full h-full rounded-[1.15rem] overflow-hidden bg-gradient-to-br from-primary-700/40 to-violet-700/40 flex items-center justify-center">
                <SafeImage
                  src={project.image}
                  ...
                  fallback={
                    <svg className="w-12 h-12 text-white/20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.3} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                    </svg>
                  }
                />
              </div>
            ...
            <div className="flex-1 pb-1 animate-fade-up" style={{ animationDelay: '.08s' }}>
              <div className="flex items-center gap-2.5 mb-4 flex-wrap">
                ...
                {isOwner && (
                  <button onClick={() => { setModalError(''); setIsEditMode(true); }} className="btn-primary btn-sm">
                    Edit
                  </button>
                )}
                {!isOwner && user && (
                  <button onClick={() => setShowCollabModal(true)} className="btn-primary btn-sm">
              ...
              <p className="eyebrow mb-1.5">Creator project</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-7 text-left">
          ...
          <div className="lg:col-span-2 space-y-7">
            <div className="glass p-7 sm:p-8">
              ...
              <div className="text-slate-300 leading-relaxed text-lg whitespace-pre-wrap">
                {project.description || <span className="italic text-slate-500">No detailed description has been provided for this project.</span>}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-7">
            <div className="glass p-7">
              ...
              <h3 className="text-lg font-bold mb-6 text-white">Project Metrics</h3>
              ...
              <div className="space-y-6">
                ...
                <div className="border-b border-white/[0.05] pb-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {safeJsonParse(project.target_aud).map((aud, idx) =>
                      aud && <span key={`aud-${idx}`} className="chip-emerald">{aud}</span>
                    )}
                    {!safeJsonParse(project.target_aud).length && <span className="text-slate-500 italic text-sm">Not specified</span>}
                  </div>
                </div>
                ...
                <div className="border-b border-white/[0.05] pb-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {!safeJsonParse(project.genre).length && <span className="text-slate-500 italic text-sm">Not specified</span>}
                  </div>
                </div>
                ...
                <div className="border-b border-white/[0.05] pb-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {safeJsonParse(project.platforms).map((p, idx) =>
                      p && <span key={`plat-${idx}`} className="chip-violet">{p}</span>
                    )}
                    {!safeJsonParse(project.platforms).length && <span className="text-slate-500 italic text-sm">Not specified</span>}
                  </div>
                </div>
