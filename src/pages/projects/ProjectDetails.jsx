      ...
      {isEditMode && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#101622] rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.5)] w-full max-w-2xl overflow-hidden border border-white/10">
            ...
            <form onSubmit={(e) => { e.preventDefault(); updateProject(); }} className="p-8 space-y-6 max-h-[80vh] overflow-y-auto">
              <div>
                <label className="block text-sm font-semibold text-slate-200 mb-2">Project Name <span className="text-red-400">*</span></label>
                <input required type="text" name="project_name" value={editFormData.project_name} onChange={handleInputChange} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all text-white placeholder-slate-500" placeholder="e.g. Project Alpha"/>
              </div>
              
              <div>
                <label className="block text-sm font-semibold text-slate-200 mb-2">Description</label>
                <textarea rows="3" name="description" value={editFormData.description} onChange={handleInputChange} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all resize-none text-white placeholder-slate-500" placeholder="What is this project about?"></textarea>
              </div>
              ...
              <div className="pt-6 mt-8 border-t border-white/10 flex justify-end gap-4 pb-2">
                ...
                <button type="submit" disabled={isSaving} className="px-6 py-2.5 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2">
        ...
        <div className="backdrop-blur-sm bg-[rgba(255,255,255,0.03)] border-b border-[rgba(255,255,255,0.05)] overflow-hidden h-72 lg:h-96">
          {project.image ? (
            <div className="absolute inset-0 z-0">
               <img src={project.image} alt={project.project_name} className="w-full h-full object-cover opacity-30 mix-blend-screen" />
        ...
        <div className="max-w-7xl mx-auto px-8 relative -mt-32 z-10 pb-8">
          <div className="flex flex-col md:flex-row gap-8 items-end">
            <div className="border-4 border-[#101622] bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl overflow-hidden shadow-2xl shrink-0 h-40 w-40 flex items-center justify-center text-6xl text-slate-600">
               {project.image ? (
                  <img src={project.image} alt={project.project_name} className="w-full h-full object-cover" />
               ) : (
                  <span>📁</span>
               )}
            </div>
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  {isOwner && (
                    <button
                      ...
                      className="text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 px-4 py-1.5 rounded-lg transition-all"
                    >
                      Edit
                    </button>
                  )}
